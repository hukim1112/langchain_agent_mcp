...
이 프로젝트에서는 **3가지 핵심 과제**를 수행합니다.

1. **MCP 서버 구축** — 기존 LangChain Retrieval 도구를 FastMCP 서버로 래핑
2. **Agent ↔ MCP 연동** — 에이전트가 MCP 서버를 동적으로 탐색·호출하도록 구현
3. **에이전트 고도화** — 장기 메모리, Middleware 등 확장 기능 적용
...
`main` 브랜치의 현재 상태에서 출발하여, 3단계 미션을 순서대로 완료하세요.
